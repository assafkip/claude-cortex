...
# Configure project-specific brand/product terms here. Empty list disables the check.
# Example: BRAND_PATTERNS = [r"Acme", r"acme", r"q-acme"]
BRAND_PATTERNS: list[str] = []
# Detects any hardcoded user home path that would break on another machine.
HARDCODED_PATH_PATTERNS = [r"/Users/[^/\s\"']+", r"/home/[^/\s\"']+"]
...
class Validator:
    ...
    def _phase_1(self, checks: list, verbose: bool = False) -> None:
        ...
        if agents_dir.is_dir():
            ...
            brand_hits = self._grep_dir(agents_dir, BRAND_PATTERNS)
            self._check(
                checks,
                "No brand-specific terms in agents",
                len(brand_hits) == 0,
                detail=", ".join(brand_hits) if brand_hits else None,
            )
        ...
        if skills_dir.is_dir():
            ...
            skill_brand_hits = self._grep_dir(skills_dir, BRAND_PATTERNS)
            self._check(
                checks,
                "No brand-specific refs in skills/",
                len(skill_brand_hits) == 0,
                detail=", ".join(skill_brand_hits) if skill_brand_hits else None,
            )

        # Full skeleton sweep
        q_system_dir = self.repo_dir / "q-system"
        if q_system_dir.is_dir():
            all_patterns = BRAND_PATTERNS + HARDCODED_PATH_PATTERNS
            sweep_hits = self._grep_dir(
                q_system_dir, all_patterns, exclude_files=SWEEP_EXCLUDE_FILES
            )
            self._check(
                checks,
                "No brand-specific/hardcoded refs in q-system/ (full sweep)",
                len(sweep_hits) == 0,
    ...
    def _phase_5(self, checks: list) -> None:
        ...
        for fname in DOC_FILES:
            ...
            if fpath.exists():
                hits = self._grep_dir(fpath.parent, BRAND_PATTERNS)
                doc_hits = [h for h in hits if Path(h).name == fname]
                self._check(
                    checks,
                    f"No brand-specific terms in {fname}",
                    len(doc_hits) == 0,
