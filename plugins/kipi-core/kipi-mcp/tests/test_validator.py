...
class TestPhase1:
    ...
    def test_phase_1_detects_brand_terms(self, tmp_path, monkeypatch):
        import kipi_mcp.validator as vmod
        monkeypatch.setattr(vmod, "BRAND_PATTERNS", [r"Acme"])
        paths, repo_dir = _build_skeleton(tmp_path)
        registry = _make_registry(tmp_path, repo_dir)
        agents_dir = (
        ...
        brand_checks = [
            ...
            if "brand" in c["description"].lower() and "agents" in c["description"]
        ]
        assert any(c["result"] == "fail" for c in brand_checks)
